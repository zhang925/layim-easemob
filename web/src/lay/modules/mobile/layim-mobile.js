...
layui.define(['laytpl', 'upload-mobile', 'layer-mobile', 'zepto'], function(exports){
    ...
    var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}}">'
        ...
        ,'{{# if(d.base && d.base.easemobImage){ }}'
        ,'<span class="layui-icon layim-tool-image" title="上传图片" layim-event="easemobImage">&#xe60d;<input type="file" name="file" accept="image/*"></span>'
        ,'{{# }; }}'
        ...
        ,'{{# }; }}'
        ,'{{# if(d.base && d.base.easemobFile){ }}'
        ,'<span class="layui-icon layim-tool-image" title="发送文件" layim-event="easemobImage" data-type="file">&#xe61d;<input type="file" name="file"></span>'
        ,'{{# }; }}'
    ...
    var anim = 'layui-anim-upbit', events = {
        ...
        //环信发送文件
        ,easemobImage:function (othis) {
            var isImage = othis.data('type') != 'file';
            var input = othis.find('input')[0];
            if (input.onchange == undefined)
                input.onchange = function (fileRef) {
                    var thatChat = thisChat();
                    var to = thatChat.data;
                    var id = easemobConn.getUniqueId();             // 生成本地消息id
                    var msg = new WebIM.message(isImage ? 'img' : 'file', id);  // 创建图片消息
                    // 单聊发送图片消息
                    var file = WebIM.utils.getFileUrl(input);      // 将图片转化为二进制文件
                    var isGroupMsg = to.type == "group";
                    var option = {
                        apiUrl: WebIM.config.apiURL,
                        file: file,
                        to: to.name,                       // 接收消息对象
                        roomType: false,
                        chatType: isGroupMsg ? 'chatRoom' : 'singleChat',
                        onFileUploadError: function (id) {      // 消息上传失败
                            layer.msg('上传失败');
                        },
                        onFileUploadComplete: function (res) {   // 消息上传成功
                            if (isImage) {
                                focusInsert(thatChat.textarea[0], 'img[' + ((res.uri + "/" + res.entities[0].uuid) || '') + ']');
                            } else {
                                focusInsert(thatChat.textarea[0], 'file(' + (res.uri + "/" + res.entities[0].uuid) + ')[' + (input.files[0].name || '下载文件') + ']');
                            }
                            sendMessage();
                        },
